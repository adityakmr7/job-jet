...
--> statement-breakpoint
-- Carry over existing app users (previously Clerk user ids) so their
-- profiles/resumes/applications keep a valid owner. They have no password
-- or linked account yet: they sign in with Google (linked by verified email)
-- or use "Forgot password". scripts/migrate-clerk-users.mjs can optionally
-- enrich these rows from a Clerk export and move them to fresh ids.
INSERT INTO "user" ("id", "name", "email", "email_verified", "created_at", "updated_at")
SELECT "id", split_part("email", '@', 1), lower("email"), false, "created_at", now() FROM "users"
